...
void IslandGen::Generate(int Width, int Height, float scale, float frequency, float amplitude, int octaves, unsigned char seed)
{
	...
	for (int y = 0; y < PerinNoise.height; y++)
	{
		for (int x = 0; x < PerinNoise.width; x++)
		{
			...
			float alpha = (float)RadialColour[index].a / 160;

			Color NoisePixel = PerlinColour[index];

			int clr = ((NoisePixel.r + NoisePixel.g + NoisePixel.b) / 3) * alpha;
